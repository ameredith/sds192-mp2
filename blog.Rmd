...
House <- house_elections %>%
  ...
  filter(state == "ME" |state =="NH" |state == "VT"|state == "MA"|state == "RI"|state == "CT"|state == "NY"| state =="NJ"| state =="PA")
...
ggplot(data= north_east) + geom_polygon( aes(x=long, y = lat, group= group ), color = "black")+coord_fixed(1.3)
